...
export class WebcartService {
  ...
  assignWebcart(userId) {
    if (userId != -1) {
      ...
      this.getWebcartByUserId(userId).subscribe(data => {
        webcart = data;
        if (webcart.webCartDetails == null || webcart.webCartDetails.length==0) {
          if (localStorage.getItem("carrito") != null) {
            var fwebcart: Webcart = JSON.parse(localStorage.getItem("carrito"))
            fwebcart.webCartDetails.forEach(detail=>{
              detail.webCart.id=webcart.id;
            })
            webcart.webCartDetails = fwebcart.webCartDetails
          } else {
      ...
      if (localStorage.getItem("carrito") != null) {
        ...
        var webcart: Webcart = new Webcart()
        webcart.id = -1;
        webcart.createdDate = new Date(Date.now());
